...
def pick_scenarios(lnsas, weights, multi=True):
  ...
  if easy == True:
    ...
    for w in weights:
      wsum += weights[w]
      if weights[w]> 0.0000001/numeps: # 0.00001/numeps: #10^-5 divided by num eps because the weights get renormalized when take more than one epsilon realization per scenario
...
def main():
  ...
  demand = bd.build_demand('input/BATS2000_34SuperD_TripTableDataDriversOnly.txt', 'input/superdistricts_centroids.csv') #bd.build_demand('input/BATS2000_34SuperD_TripTableData.csv', 'input/superdistricts_centroids.csv')
  #get earthquake info
