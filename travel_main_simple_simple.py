...
def main():
  ...
  demand = bd.build_demand('input/BATS2000_34SuperD_TripTableData.csv', 'input/superdistricts_centroids.csv') #we just take a percentage in ita.py, namely  #to get morning flows, take 5.3% of daily driver values. 11.5/(4.5*6+11.5*10+14*4+4.5*4) from Figure S10 of http://www.nature.com/srep/2012/121220/srep01001/extref/srep01001-s1.pdf
          #get path
  #get earthquake info #UPDATED November 26, 2013
  ...
  numeps = 3 #CAHNGE THIS CHANGE THIS!!!!!!!!
...
#      print 'new travel times: ', travel_index_times
    if index%1000 ==0:
      print 'index: ', index
      util.write_2dlist(time.strftime("%Y%m%d")+'_bridges_flow_paths_' + str(numeps) + 'eps_extensive.txt',travel_index_times)
    index += 1 #IMPORTANT
  util.write_2dlist(time.strftime("%Y%m%d")+'_bridges_flow_paths_' + str(numeps) + 'eps_extensive.txt',travel_index_times)
  ...
  util.write_2dlist(time.strftime("%Y%m%d")+'_damaged_bridges_' + str(numeps) + 'eps_extensive.txt',BRIDGE_DAMAGE_DATASET)
  with open(time.strftime("%Y%m%d")+'_damaged_bridges_' + str(numeps) + 'eps_extensive.pkl', 'wb') as f:
